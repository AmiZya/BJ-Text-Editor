 ...
 * defines
 */
#define BJ_VERSION "0.0.1"
#define BJ_TAB_STOP 8
#define BJ_QUIT_TIMES 2
...
int editorRowCxToRx(erow *row, int cx){
    ...
    for(j=0; j < cx; j++){
        if(row->chars[j] == '\t')
            rx += (BJ_TAB_STOP - 1) - (rx % BJ_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx){
    ...
    for(cx = 0; cx < row->size; cx++){
        if(row->chars[cx] == '\t')
            cur_rx += (BJ_TAB_STOP - 1) - (cur_rx % BJ_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row){
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(BJ_TAB_STOP - 1) + 1);

    int idx = 0;
    for(j=0; j < row->size; j++){
        if(row->chars[j] == '\t'){
            row->render[idx++] = ' ';
            while(idx % BJ_TAB_STOP != 0) row->render[idx++] = ' ';
        }else{
...
void editorDrawRows(struct abuf *ab){
    ...
    for(y=0; y<E.screenrows; y++){
        ...
        if(E.numrows ==0 && y == E.screenrows / 3){
            char welcome[80];
            int welcomelen = snprintf(welcome, sizeof(welcome), "BJ text editor -- version %s", BJ_VERSION);
...
void editorDrawStatusBar(struct abuf *ab){
    ...
    int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
                     E.filename ? E.filename : "[A File Has No Name]", E.numrows,
                     E.dirty ? "(modified)" : "");
...
void editorProcessKeypress(){
    static int quit_times = BJ_QUIT_TIMES;
    int c = editorReadKey();
